...
<form method="post">
	...
	<button type="button" id="btn2">이메일 인증</button>
...
<script>
	...
	$('#btn').on('click',function(){
		...
		//아이디 올바르게 쓰지 않았을 때 바로 리턴
		if(id==null || id=='') return $('#idcheck').text('올바른 이메일을 입력 해 주세요');
		//ajax로 id보내기 -> emailCheck 매핑된 곳으로 보내서 중복 확인
		$.ajax({
			url : 'idCheck',
			type : 'post',
			data : {'id' : id}, // 입력한 email data
			dataType : 'text', 
